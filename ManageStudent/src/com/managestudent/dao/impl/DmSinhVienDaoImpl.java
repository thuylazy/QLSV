	...
	public List<DmSinhVien> getAllSinhVien(DmSinhVien sinhVien, int offset, int limit, int sortColumn, String sortType) {
		...
		if(connectToDB()) {
			try {
				...
				sqlCommand.append("sv.hedt_id, sv.lop_id, sv.khoahoc_id, hdt.ten_hedt, l.tenlop, kh.tenkhoahoc ");
				sqlCommand.append("FROM dmsinhvien sv ");
				sqlCommand.append("INNER JOIN hedaotao hdt ");
				sqlCommand.append("ON sv.hedt_id = hdt.hedt_id ");
				...
				if(sinhVien.getTenHeDaoTao().length() > 0) {
					...
					sqlCommand.append("hdt.ten_hedt LIKE ? ");
					conCount++;
				}
				if(sinhVien.getMaHeDaoTao().length() > 0) {
					if(conCount > 0) {
						sqlCommand.append("AND ");
					} else {
						sqlCommand.append("WHERE ");
				...
				if((pos = sqlCommand.indexOf("sv.hedt_id = ?")) > 0) {
					tmp = sqlCommand.substring(0, pos);
					countChar = tmp.length() - tmp.replace("?", "").length();
					preparedStatement.setInt(countChar + 1, sinhVien.getHeDtId());
				}
				if((pos = sqlCommand.indexOf("hdt.ten_hedt LIKE ?")) > 0) {
					tmp = sqlCommand.substring(0, pos);
					countChar = tmp.length() - tmp.replace("?", "").length();
					preparedStatement.setString(countChar + 1, "%" + sinhVien.getTenHeDaoTao() + "%");
				}
	...
	public List<DmSinhVien> getListSinhVienByHeDaoTaoId(int heDaoTaoId, DmSinhVien sinhVien, int offset, int limit, int sortColumn, String sortType) {
		...
		if(connectToDB()) {
			try {
				...
				sqlCommand.append("WHERE sv.hedt_id = ? ");
				if(sinhVien.getMaSinhVien().length() > 0) {
				...
				if(sinhVien.getTenHeDaoTao().length() > 0) {
					sqlCommand.append("AND hdt.ten_hedt LIKE ? ");
				}
				if(sinhVien.getMaHeDaoTao().length() > 0) {
					sqlCommand.append("AND hdt.mahedt LIKE ? ");
				}
				if(sinhVien.getTenLopHoc().length() > 0) {
	...
	public List<DmSinhVien> getAllSinhVienDetail(DmSinhVien sinhVien, int offset, int limit, int sortColumn, String sortType) {
		...
		if(connectToDB()) {
			try {
				...
				sqlCommand.append("sv.chedouudai. sv.dantoc_id, sv.tongiao_id, sv.quoctich_id, sv.hotenbo, sv.nghenghiepbo, ");
				...
				if(sinhVien.getQuocTichId() > 0) {
					...
					sqlCommand.append("sv.quoctich_id = ? ");
					conCount++;
				}
