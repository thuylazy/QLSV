...
/**
 * Servlet implementation class DanTocDetailController
 */
public class DanTocDetailController extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public DanTocDetailController() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(Common.checkLogin(request.getSession())) {
			if(request.getParameter("id") != null && request.getParameter("id").length() > 0) {
				try {
					int danTocId = Integer.parseInt(request.getParameter("id"));
					DanTocLogicsImpl danTocLogics = new DanTocLogicsImpl();
					DanToc danToc = null;

					if((danToc = danTocLogics.getDanTocById(danTocId)) != null) {
						request.getSession().setAttribute("dantoc", danToc);
						request.setAttribute("id", request.getParameter("id"));
						request.setAttribute("ref", request.getParameter("ref"));
						template = Constant.DANTOCDETAIL;
					} else {
